...
export const getPlayerById = createServerFn({ method: "GET" }).inputValidator(z.object({
...
export const deletePlayer = createServerFn({ method: "POST" }).inputValidator(z.object({
  playerId: z.string().uuid()
})).handler(async ({ data }) => {
  try {
